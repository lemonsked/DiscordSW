...
public class BTC_MainPanel extends javax.swing.JFrame
{
    ...
    static final String path = "ftclogs/";
    public void logTelnetMessage(BTC_ConsoleMessage msg)
    {

        if(BTC_ConnectionManager.hostname == null)
        {
            return;
        }
        LocalDate date = LocalDate.now();
        String filename = path + BTC_ConnectionManager.hostname + "/" + date.getDayOfMonth() + "-" + date.getMonth() + "-" + date.getYear() +   ".log";
        ...
        try
        {
            if(file.exists() && !isFileEmpty(file))
            {
                ...
                {
                    String content;
                    if(msg.getMessage().contains("Connecting to "))
                    {
                        return;
                    }
                   else if (msg.getMessage().contains(":[BukkitTelnet]$ Session Started.") || msg.getMessage().equals("Connection throttled. Please wait a minute and try again."))
                    {
                        content = "\n\nReconnected.\n\n" + msg.getMessage() + "\n";
                    }
                    else
